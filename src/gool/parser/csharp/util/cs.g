...
compilation_unit returns [CsharpNode t]:
...
namespace_body[boolean bGlobal] returns [CsharpNode t]:
...
extern_alias_directives returns [UnknowNode t]:
        extern_alias_directive+ {$t=new UnknowNode(root_0);};
extern_alias_directive:
        'extern'   'alias'   identifier  ';' ;
using_directives returns [UnknowNode t]:
        using_directive+ {$t=new UnknowNode(root_0);};
...
namespace_member_declarations returns [CsharpNode t]:
        (a=namespace_member_declaration  { if ($t == null) {$t=new namespace_member_declarations(); ((namespace_member_declarations)$t).add($a.t);} else {((namespace_member_declarations)$t).add($a.t);} } )+ ;
        
namespace_member_declaration returns [CsharpNode t]:
        a=namespace_declaration {$t=new UnknowNode(a);}
...
type_declaration returns [CsharpNode t]:
        ...
        | u=enum_declaration
        | u=delegate_declaration) {if (a != null) $t=new type_declaration($a.t); else $t=new type_declaration(new UnknowNode(u));} ;
...
modifiers  returns [UnknowNode t]:
        modifier+ {$t = new UnknowNode(root_0);} ;
modifier: 
...
class_member_declaration returns [CsharpNode t]:
        b=attributes?
        c=modifiers?
        ( 'const'   type   constant_declarators   ';' {/*TODO*/}
        | 'partial' (method_declaration 
                           | interface_declaration 
                           | class_declaration 
                           | struct_declaration) {/*TODO*/}
        | e='void'  aa=method_declaration {$t=new class_member_declaration_type($b.t,$c.t,new TokenNode(e),$aa.t);}
        | d=type ( (member_name   '(') => aa=method_declaration {$t=new class_member_declaration_type($b.t,$c.t,$d.t,$aa.t);}
                   | (member_name   '{') => property_declaration
                   | (member_name   '.'   'this') => type_name '.' indexer_declaration
               | u=indexer_declaration        //this
               | u=field_declaration      // qid
               | u=operator_declaration
               ) 
               {{if (a != null) $t=new class_member_declaration_type($b.t,$c.t,$d.t,$a.t); 
               else $t=new class_member_declaration_type($b.t,$c.t,$d.t,new UnknowNode(u));}}
//        common_modifiers// (method_modifiers | field_modifiers)
        
        | (a=class_declaration                // 'class'
        | u=struct_declaration        // 'struct'           
        | u=enum_declaration  
        | u=event_declaration                // 'event' 
        | u=interface_declaration        // 'interface'
        | u=delegate_declaration        // 'delegate'
        | u=conversion_operator_declaration
        | u=constructor_declaration        //        | static_constructor_declaration
        | u=destructor_declaration) 
        {{if (a != null) $t=new class_member_declaration($b.t,$c.t,$a.t); 
        else $t=new class_member_declaration($b.t,$c.t,new UnknowNode(u));}}
        ) 
        ;
...
array_initializer  returns [UnknowNode t]:
        '{'   variable_initializer_list?   ','?   '}'  {$t=new UnknowNode(root_0);}
;
...
type_or_generic returns [UnknowNode t]:
        ((identifier   '<') => identifier   generic_argument_list
        | identifier ){$t=new UnknowNode(root_0);};
...
//        Statement Section
///////////////////////////////////////////////////////
block  returns [CsharpNode t]:
        ';'
        | '{'   a=statement_list?   '}' {$t=$a.t;} ;
statement_list returns [CsharpNode t]:
       ( a=statement  { if ($t == null) {$t=new statement_list(); ((statement_list)$t).add($a.t);} else {((statement_list)$t).add($a.t);} } )+  ;
        
///////////////////////////////////////////////////////
//        Expression Section
///////////////////////////////////////////////////////        
expression returns [CsharpNode t]:
        ((unary_expression   assignment_operator) => assignment        
        | non_assignment_expression) {$t=new UnknowNode(root_0);}
...
// B.2.13 Attributes
///////////////////////////////////////////////////////
global_attributes returns [UnknowNode t]:
        global_attribute+ {$t=new UnknowNode(root_0);};
...
global_attribute_target: 
        'assembly' | 'module' ;
attributes returns [UnknowNode t]: 
        (attribute_sections ){$t=new UnknowNode(root_0);};
attribute_sections: 
        attribute_section+ ;
attribute_section: 
...
class_declaration  returns [CsharpNode t]:
...
class_base  returns [UnknowNode t]:
        // syntactically base class vs interface name is the same
        //':'   class_type (','   interface_type_list)? ;
        (':'   interface_type_list) {$t=new UnknowNode(root_0);};
        
interface_type_list:
        type (','   type)* ;

class_body   returns [CsharpNode t]:
        '{'   a=class_member_declarations?   '}' {$t = new class_body ($a.t); };
class_member_declarations  returns [CsharpNode t]:
        (a=class_member_declaration 
         { if ($t == null) {$t=new class_member_declarations(); ((class_member_declarations)$t).add($a.t);} else {((class_member_declarations)$t).add($a.t);} })+ ;

///////////////////////////////////////////////////////
constant_declaration:
        'const'   type   constant_declarators   ';' ;
constant_declarators  returns [UnknowNode t]:
        constant_declarator (',' constant_declarator)*  {$t=new UnknowNode(root_0);} ;
constant_declarator:
...
type_parameter_constraints_clauses  returns [UnknowNode t]:
        type_parameter_constraints_clause   (','   type_parameter_constraints_clause)* {$t=new UnknowNode(root_0);};
...
return_type:
        type
        |  'void';
formal_parameter_list  returns [UnknowNode t]:
        formal_parameter (',' formal_parameter)* {$t=new UnknowNode(root_0);};
formal_parameter:
...
embedded_statement returns [CsharpNode t]:
        ...
        | unsafe_statement
        | fixed_statement
        | expression_statement ) {$t=new UnknowNode(root_0);}
       // expression!
        ;
fixed_statement:
...
unsafe_statement:
        'unsafe'   block;
labeled_statement returns [CsharpNode t]:
        (identifier   ':'   statement)  {$t=new UnknowNode(root_0);};
declaration_statement returns [CsharpNode t]:
        (b=local_variable_declaration {$t=$b.t;}
        | c=local_constant_declaration {$t=$c.t;} ) ';' ;

local_variable_declaration returns [CsharpNode t]:
        a=local_variable_type   b=local_variable_declarators  
        {$t = new local_variable_declaration($a.t,$b.t); System.out.println($t.toStringTree(3));};
local_variable_type returns [CsharpNode t]:
        ('var') => a='var' {$t=new TokenNode($a);}
        | ('dynamic') => b='dynamic' {$t=new TokenNode($b);}
        | c=type {$t=$c.t;} ;
local_variable_declarators  returns [CsharpNode t]:
        a=local_variable_declarator {$t=new local_variable_declarators(); ((local_variable_declarators)$t).add($a.t);} (',' a=local_variable_declarator {((local_variable_declarators)$t).add($a.t);} )* ;
local_variable_declarator returns [CsharpNode t]:
        a=identifier ('='   b=local_variable_initializer)?  {$t=new local_variable_declarator($a.t,$b.t);} ; 
local_variable_initializer returns [CsharpNode t]:
        a=expression {$t=$a.t;}
        | b=array_initializer {$t=$b.t;}
        | c=stackalloc_initializer  {$t=$c.t;};
stackalloc_initializer  returns [UnknowNode t]:
        'stackalloc'   unmanaged_type   '['   expression   ']'  {$t=new UnknowNode(root_0);};
local_constant_declaration returns [CsharpNode t]:
...
identifier returns [CsharpNode t]:
